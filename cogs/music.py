...
class Music(commands.Cog):
    ...
    async def play_autocomplete(
        ...
        """Search YouTube and show 5 results as autocomplete suggestions."""
        ...
        try:
            ...
            data = await asyncio.get_event_loop().run_in_executor(
                None, lambda: ydl.extract_info(f"ytsearch5:{current}", download=False)
            )
            ...
            if data and 'entries' in data:
                for entry in data['entries'][:5]:
                    ...
                    # Build display: "Title (M:SS) - Uploader"
                    display = f"{title} {dur_str}"
                    if uploader:
                        display += f" - {uploader}"
